-- Create positions table for tracking managed bot positions
-- Depends on: alerts table (20260212000000)

-- Create enum types for position state and side
...
CREATE POLICY "Service role has full access" ON positions
  FOR ALL
  USING (true)
  WITH CHECK (true);

-- Add comment to table
COMMENT ON TABLE positions IS 'Bot-managed positions with VPVR levels and trailing TP/SL state';
