...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  if (!validateWebhookSecret(secret)) {
    logger.warn('Invalid webhook secret on informational endpoint');
  ...
  try {
    const eventId = await saveRawWebhook('informational_events', {
    ...
    logger.error('Failed to store informational webhook', { error: message });
