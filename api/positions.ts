...
import type { PositionRow, PositionState, PositionSide } from '../src/types/database';
...
const VALID_STATES: PositionState[] = [
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  const state = req.query.state ? String(req.query.state) as PositionState : undefined;
  ...
  try {
    ...
    const response: PositionsResponse = { success: true, data: data ?? [], pagination };
